...
<div class="container">
	<div class="baseContent">
		<h2 style="font-size: 1.3em; margin-bottom: 10px;">나의 후기내역</h2>
		<table class="user_table">
			...
			<c:choose>
				<c:when test="${!empty(myReviewAllList) }">
					<c:forEach items="${myReviewAllList }" var="myReviewAll">
						<tr onclick="location.href='${pageContext.request.contextPath}/review_detail?rno=${myReviewAll.rno}'"  class="moveToDetail">
							<td>${myReviewAll.rno }</td>
							<td>${myReviewAll.rtitle }</td>
							<td><fmt:formatDate value="${myReviewAll.rdate }" pattern="yyyy-MM-dd"/></td>				
							<%-- <c:choose>
								<c:when test="${myReviewAll.rstate==1 }">
									<td>미답변</td>
								</c:when>
								<c:when test="${myReviewAll.rstate==2 }">
									<td>답변완료</td>
								</c:when>
								<c:otherwise>
									<td>구현중</td>
